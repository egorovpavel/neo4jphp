...
class AddToIndexUnique extends AddToIndex
{
	...
	protected function getData()
	{
		...
		$data['value'] = $this->value;
                ...
                }elseif ($this->index->getType() === Index::TypeRelationship) {
                    $data['start'] = $this->getTransport()->getEndpoint().'/node/'.$this->entity->getStartNode()->getId();
                    $data['end'] = $this->getTransport()->getEndpoint().'/node/'.$this->entity->getEndNode()->getId();
                    $data['type'] = $this->entity->getType();
                    $data['properties'] = $this->entity->getProperties();
                }
		return $data;
	}
	...
	protected function handleResult($code, $headers, $data)
	{
		if ($code === 200 || $code === 201) {
                        if($this->index->getType() === Index::TypeRelationship){
                            $this->getEntityMapper()->populateRelationship($this->entity, $data);
                        }else{
		...
		if ($code === 409) {
			$this->throwException('Entity already exists!', $code, $headers, $data);
                        return false;
		}
